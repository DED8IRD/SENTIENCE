...
pages = [line.strip() for line in open(os.path.join(mydir, "page_names"), 'r')]
...
def getFacebookPageFeedData(page_id, access_token, num_statuses):
    ...
    if request_until_succeed(url):
        data = json.loads(request_until_succeed(url))
    else:
        data = False
        with open('rip_pages', 'a+') as file:
            file.write(page_id) 
...
def scrapeFacebookPageFeedStatus(page_id, access_token):
    filename = os.path.join(mydir, page_id + '_facebook_statuses.csv')
    existing = None
    try:
    ...
    with open(filename, 'wb') as file: 
        ...
        while has_next_page and not has_existing_post and statuses:
